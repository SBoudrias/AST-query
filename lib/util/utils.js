...
exports.matchRegexMemberExpression = function matchRegexMemberExpression(nameRegex, node) {
  ...
  if (node.type === 'MemberExpression') {
    ...
    return matchRegexMemberExpression(node.object);
  }

  return !!(node.type === 'Identifier' && nameRegex.test(nameRegex));
};
...
exports.isRegEx = function (possibleRegEx) {
  return possibleRegEx.constructor.name === 'RegExp';
};
